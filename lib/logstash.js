...
LogstashStream.prototype.write = function logstashWrite(entry) {
  ...
  // add meta to log message
  if((entry.meta) && (typeof(entry.meta) === "object")) {
    for(var prop in entry.meta) {
      msg[prop] = entry.meta[prop];
    }
  }
  ...
  var sndObj = _.extend({}, msg, rec);
  delete sndObj.meta;

  this.send(JSON.stringify(sndObj), bunyan.safeCycles());
};
